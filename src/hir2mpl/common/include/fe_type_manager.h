 ...
 */
#ifndef HIR2MPL_INCLUDE_COMMON_FE_TYPE_MANAGER_H
#define HIR2MPL_INCLUDE_COMMON_FE_TYPE_MANAGER_H
#include <map>
#include <memory>
#include <list>
...
class FETypeManager {
 public:
  ...
  void InsertOwnedNonnullFieldStructSet(const TyIdx &tyidx) {
    (void)ownedNonnullFieldStructSet.insert(tyidx);
  }

  bool IsOwnedNonnullFieldStructSet(const TyIdx &tyidx) {
    auto iter = ownedNonnullFieldStructSet.find(tyidx);
    if (iter != ownedNonnullFieldStructSet.end()) {
      return true;
    }
    return false;
  }
 ...
 private:
  ...
  std::unordered_set<TyIdx, TyIdxHash> ownedNonnullFieldStructSet;
...
#endif  // HIR2MPL_INCLUDE_COMMON_FE_TYPE_MANAGER_H
