...
TEST_F(JBCStack2FEHelperTest, GenerateSwapStmts_Test1) {
  ...
  std::string pattern = std::string("dassign %Reg5_D 0 \\(dread f64 %Reg3_D\\)") + HIR2MPLUTRegx::Any();
  ...
  pattern = std::string("dassign %Reg7_J 0 \\(dread i64 %Reg1_J\\)") + HIR2MPLUTRegx::Any();
  EXPECT_EQ(HIR2MPLUTRegx::Match(dumpStr, pattern), true);
  ...
  dumpStr = GetBufferString();
  pattern = std::string("dassign %Reg9_I 0 \\(dread i32 %Reg0_I\\)") + HIR2MPLUTRegx::Any();
  EXPECT_EQ(HIR2MPLUTRegx::Match(dumpStr, pattern), true);
  RestoreCout();
}
...
TEST_F(JBCStack2FEHelperTest, LoadSwapStack_Test1) {
  ...
  mirStmts0.front()->Dump();
  std::string dumpStr = GetBufferString();
  std::string pattern = std::string("dassign %Reg0_I 0 \\(dread i32 %Reg9_I\\)") + HIR2MPLUTRegx::Any();
  EXPECT_EQ(HIR2MPLUTRegx::Match(dumpStr, pattern), true);
  ...
  mirStmts1.front()->Dump();
  dumpStr = GetBufferString();
  pattern = std::string("dassign %Reg1_J 0 \\(dread i64 %Reg7_J\\)") + HIR2MPLUTRegx::Any();
  EXPECT_EQ(HIR2MPLUTRegx::Match(dumpStr, pattern), true);
  ...
  mirStmts2.front()->Dump();
  dumpStr = GetBufferString();
  pattern = std::string("dassign %Reg3_D 0 \\(dread f64 %Reg5_D\\)") + HIR2MPLUTRegx::Any();
  EXPECT_EQ(HIR2MPLUTRegx::Match(dumpStr, pattern), true);
  RestoreCout();
}
}  // namespace maple