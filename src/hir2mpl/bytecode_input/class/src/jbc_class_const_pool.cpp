...
const JBCConst *JBCConstPool::GetConstByIdx(uint16 idx, bool safed) const {
  if (idx == 0 || idx >= pool.size()) {
    std::stringstream ss;
    ss << "invalid const pool idx " << static_cast<uint32>(idx) << ".";
    CHECK_FATAL(!safed, "%s", ss.str().c_str());
...
const JBCConst *JBCConstPool::GetConstByIdxWithTag(uint16 idx, JBCConstTag tag, bool safed) const {
  const JBCConst *obj = GetConstByIdx(idx, safed);
  if (obj != nullptr && obj->GetTag() != tag) {
    ...
    CHECK_FATAL(!safed, "%s Exit.", ss.str().c_str());
    ERR(kLncErr, "%s", ss.str().c_str());
    return nullptr;
  }
  return obj;
...
const JBCConst *JBCConstPool::GetConstValueByIdx(uint16 idx, bool safed) const {
  const JBCConst *obj = GetConstByIdx(idx, safed);
...
const JBCConst *JBCConstPool::GetConstValue4ByteByIdx(uint16 idx, bool safed) const {
  const JBCConst *obj = GetConstByIdx(idx, safed);
...
const JBCConst *JBCConstPool::GetConstValue8ByteByIdx(uint16 idx, bool safed) const {
  const JBCConst *obj = GetConstByIdx(idx, safed);
...
std::string JBCConstPool::GetNameByClassInfoIdx(uint16 idx, bool safed) const {
  const JBCConstClass *constClass = static_cast<const JBCConstClass*>(GetConstByIdxWithTag(idx, kConstClass, safed));
