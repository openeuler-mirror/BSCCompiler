...
astdump_init = [
        ...
        '',
        'public:',
        gen_args[1] + '(ASTModule *m) : mASTModule(m), mOs(nullptr), indent(0) {',
        ...
        'indent += 4;', '*mOs << " {" << std::endl;', 'DumpTreeNode(node);',
        '}', 'return node;',
        '}', '',
        'void DumpFE() {',
        'indent -= 4;', '*mOs << indstr.substr(0, indent + 2) << "}" << std::endl;',
        '}', '',
        'void DumpLB(const std::string& msg, unsigned size) {',
        '*mOs << indstr.substr(0, indent) << msg << size << (size ? " [" : "") << std::endl;', 'indent += 4;',
        '}', '',
        'void DumpLE(unsigned size) {',
        'indent -= 4;',
        'if(size)', '*mOs << indstr.substr(0, indent + 2) << "]" << std::endl;',
        '}', '',
        ]
...
astgraph_init = [
        gen_args[1] + '(ASTModule *m) : mASTModule(m), mOs(nullptr) {}', '',
        ...
        'case NK_Identifier:  *mOs << "\\\\\\"" << n->GetName() << "\\\\\\""; break;',
        'case NK_Decl:        *mOs << EnumVal(DeclNode, DeclProp, Prop, 0); break;',
        'case NK_PrimType:    *mOs << EnumVal(PrimTypeNode, TypeId, PrimType, 0); break;',
        'case NK_BinOperator: *mOs << EnumVal(BinOperatorNode, OprId, OprId, 4); break;',
        'case NK_UnaOperator: *mOs << EnumVal(UnaOperatorNode, OprId, OprId, 4); break;',
