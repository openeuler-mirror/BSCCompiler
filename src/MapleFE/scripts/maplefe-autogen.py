...
class {gen_args1} {{
public:""".format(gen_args1upper=gen_args[1].upper(), gen_args1=gen_args[1], gen_args3=gen_args[3])
]
...
#endif""".format() # Use format() to match each pair of "{{" and "}}"
]
...
void PutChildEdge(TreeNode *from, TreeNode *to, const char *field, unsigned idx, NodeKind k) {{
  if(to) {{
    *mOs << NodeName(from,\'_\') << " -> " << NodeName(to,\'_\') << "[label=\\"" << field << "[" << idx << "]\\"" << (to->GetKind() == k || k == NK_Null ? "" : ", style=bold, color=red") << "];\\n";
  }}
}}
...
ASTModule            *mASTModule;
std::ostream         *mOs;
std::set<TreeNode *>  mNodes;""".format(gen_args1=gen_args[1], gen_args2=gen_args[2], astdumpclass=astdumpclass)
        ]
