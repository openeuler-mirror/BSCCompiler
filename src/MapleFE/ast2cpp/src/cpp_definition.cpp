...
std::string CppDef::EmitDeclNode(DeclNode *node) {
  ...
  if (auto n = node->GetVar()) {
    ...
    // if cls obj get cls name
    if (n->IsIdentifier() && n->GetTypeId() == TY_Class) {
      clz = EmitTreeNode(static_cast<IdentifierNode*>(n)->GetType());
    }
  }
  if (auto n = node->GetInit()) {
    if (node->GetTypeId() == TY_Class && n->IsStructLiteral()) {
      std::string props = EmitTreeNode(n);            // build obj props with obj literal
      std::string ctor = clz.empty()? "Object" : clz; // call Object_ctor if no class info
      str += "  "s + name + " = "s + ctor + "_ctor._new("s + props +")"s;
    }
