...
std::string CppDef::EmitObjWithProps(std::string varName, TreeNode* varIdType, std::string props) {
  if (varName.empty())
    return std::string();
...
std::string CppDef::EmitDeclNode(DeclNode *node) {
  ...
  // Skip because decl and init of array objs generated by cpp_decl already
  ...
  if (auto n = node->GetInit()) {
    if (n->IsStructLiteral())
      str += EmitObjWithProps(varStr, idType, EmitTreeNode(n));   // init with props from obj literal
