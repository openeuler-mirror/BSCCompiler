...
std::string EmitObjWithProps(std::string objName, std::string className, std::string props) {
  if (objName.empty())
    return std::string();

  std::string str;
  if (className.empty())
    str = "  "s +objName+ " = Object_ctor._new("s +props+")"s;
  else if (className.compare(0, 7, "Record<") == 0)
    str = "  "s +objName+ " = new "s +className+ "(&Object_ctor, Object_ctor.prototype, "s +props+")"s;
  else
    str = "  "s +objName+ " = "s +className+ "_ctor._new("s +props+")"s;
  return str;
}
...
std::string CppDef::EmitDeclNode(DeclNode *node) {
  ...
  if (auto n = node->GetVar()) {
    ...
    // already declared in EmitFuncSCopeVarDecls. For both cases, emit var name.
    // Otherwise for function var of JS_Let/JS_Const, emit both var type & name
    if (isInit || node->GetProp() == JS_Var)
      name = EmitTreeNode(n);             // emit var name only
    else
      name += mCppDecl.EmitTreeNode(n);   // emit both var type and name

    if (n->IsIdentifier() && n->GetTypeId() == TY_Class) {
      if (static_cast<IdentifierNode*>(n)->GetType() &&
          static_cast<IdentifierNode*>(n)->GetType()->IsUserType())
        className = EmitTreeNode(static_cast<IdentifierNode*>(n)->GetType());
    }
  }
  if (auto n = node->GetInit()) {
    if (node->GetTypeId() == TY_Class && n->IsStructLiteral())
      str += EmitObjWithProps(name, className, EmitTreeNode(n)); // init with obj literal
    else if (node->GetVar()->IsIdentifier() && n->IsIdentifier() && n->GetTypeId() == TY_Class)
      str += name + "= &"s + n->GetName() + "_ctor"s;            // init with ctor address
    else
