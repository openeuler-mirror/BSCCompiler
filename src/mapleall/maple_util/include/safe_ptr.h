...
class SafePtr {
 ...
 public:
  ...
  explicit SafePtr(pointer ptr) : base(ptr) {}

  explicit SafePtr(T &ref) : base(ref, CheckNothing<T>) {}

  SafePtr(T &&ref) = delete;

  template<typename U, typename = std::enable_if_t<std::is_convertible<U, T>::value>>
  explicit SafePtr(U *ptr) : base(ptr) {}

  template<typename U, typename = std::enable_if_t<std::is_convertible<U, T>::value>>
  explicit SafePtr(U &ref) : base(ref, CheckNothing<T>) {}
