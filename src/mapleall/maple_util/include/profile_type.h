...
struct FuncCounterItem {
  uint32_t callTimes;
  explicit FuncCounterItem(uint32_t callTimes) : callTimes(callTimes) {}
};

struct MetaItem {
  uint32_t idx;
  explicit MetaItem(uint32_t idx) : idx(idx) {}
};
...
constexpr uint8_t kProfileMagic[] = { 'm', 'a', 'p', 'l', 'e', '.', 'p', 'r', 'o', 'f', 'i', 'l', 'e', '\0' };
constexpr uint8_t kVer[] = { 0, 0, 1 };
