...
namespace maple {
  ...
  MIRAddrofConst *createSourceLocConst(MIRModule &mirModule, MIRSymbol *Var, PrimType primType) {
    ...
    MIRConst *LocData[] = {
            ...
            GlobalTables::GetIntConstTable().GetOrCreateIntConst(
                    Var->GetSrcPosition().LineNum(), *GlobalTables::GetTypeTable().GetInt32()),
            GlobalTables::GetIntConstTable().GetOrCreateIntConst(
                    Var->GetSrcPosition().Column(), *GlobalTables::GetTypeTable().GetInt32()),
    };
    // Create struct type
    MIRStructType LocStruct(kTypeStruct);
    GlobalTables::GetTypeTable().AddFieldToStructType(
            LocStruct, "module_name", *GlobalTables::GetTypeTable().GetTypeFromTyIdx((TyIdx) primType));
    GlobalTables::GetTypeTable().AddFieldToStructType(
            LocStruct, "line", *GlobalTables::GetTypeTable().GetInt32());
    GlobalTables::GetTypeTable().AddFieldToStructType(
            LocStruct, "column", *GlobalTables::GetTypeTable().GetInt32());
    ...
    MIRSymbol *LocStructSym = mirModule.GetMIRBuilder()->CreateSymbol(
            LocStructTy, Var->GetName() + "_Loc", kStConst, kScGlobal, nullptr, kScopeGlobal);
  ...
  MIRAddrofConst *createAddrofConst(MIRModule &mirModule, MIRSymbol *mirSymbol, PrimType primType) {
    ...
    MIRAddrofConst *mirAddrofConst = mirModule.GetMemPool()->New<MIRAddrofConst>(
            addrofNode->GetStIdx(), addrofNode->GetFieldID(),
            *GlobalTables::GetTypeTable().GetTypeFromTyIdx((TyIdx) primType));
    return mirAddrofConst;
  }

  // Create a constant for Str so that we can pass it to the run-time lib.
  MIRStrConst *createStringConst(MIRModule &mirModule,
                                            std::basic_string<char> Str, PrimType primType) {
    MIRStrConst *strConst = mirModule.GetMemPool()->New<MIRStrConst>(
            GlobalTables::GetUStrTable().GetOrCreateStrIdxFromName(Str),
            *GlobalTables::GetTypeTable().GetTypeFromTyIdx((TyIdx) primType));
  ...
  bool isTypeSized(MIRType *type) {
    if (type->GetKind() == kTypeScalar
        || type->GetKind() == kTypePointer || type->GetKind() == kTypeBitField) {
      return true;
    }
    if (type->GetKind() != kTypeStruct && type->GetKind() != kTypeStructIncomplete
        && type->GetKind() != kTypeArray && type->GetKind() != kTypeFArray
        && type->GetKind() != kTypeUnion) {
      return false;
    }
  ...
  MIRSymbol *getOrCreateSymbol(MIRBuilder *mirBuilder, TyIdx tyIdx, const std::string &name,
                               MIRSymKind mClass, MIRStorageClass sClass, MIRFunction *func, uint8 scpID) {
...
CallNode *retCallCOV(MeFunction &func,int bb_id, int stmt_id,int br_true,int type_of_check){
    ...
    MIRFunction *__san_cov_trace_pc = getOrInsertFunction(
        builder, "__san_cov_trace_pc", voidType, {});
...
std::set<Opcode> OP_code_blacklist{OP_addroffunc, OP_iaddrof,
                                   OP_addrof, OP_iread, 
                                   OP_ireadoff, OP_iassign,
                                   OP_dread, OP_regread, 
                                   OP_regassign, OP_dassign,
                                   OP_maydassign,OP_iassignoff,
                                   OP_iassignfpoff,
...
set_check commit(set_check old,set_check latest){
  ...
  old.register_terminal.insert( old.register_terminal.end(), latest.register_terminal.begin(), latest.register_terminal.end() );
