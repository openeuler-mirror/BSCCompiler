...
namespace maple {
  ...
  void doInstrumentAddress(AddressSanitizer *Phase, StmtNode *I,
                           StmtNode *InsertBefore, BaseNode *Addr,
  ...
  bool AddressSanitizer::instrumentFunction(MeFunction &func) {
    ...
    if (numInstrumented > 0 || changedStack || !noReturnCalls.empty()) {
      ...
      if(check_env>0){
              ...
              // 2. callassigned
                ...
                if(userchecks.count(&stmt)>0){
                  ...
                  if (CallNode *testcallNode = dynamic_cast<CallNode *>(nextStmt)){
                    MIRFunction *testcalleeFunc = GlobalTables::GetFunctionTable().GetFunctionFromPuidx(testcallNode->GetPUIdx());
                ...
                if(UC_SC_mapping[san_i_stmt_ID].count(user_j_stmt_ID)){
                  ...
                  if(sat_check(user_set_check[user_j],san_set_check[san_i])){
                    goflag= true;
                      ...
                      san_set_check[san_i].opcode.erase(std::remove_if(san_set_check[san_i].opcode.begin(), san_set_check[san_i].opcode.end(), isBlacklist), san_set_check[san_i].opcode.end());
                      if(sat_check(user_set_check[user_j],san_set_check[san_i])){
                        goflag= true;
                      }
                  }
        ...
        if ((func.GetName().compare("main") == 0)&&(check_env==1)){
          ...
          __san_cov_flush = getOrInsertFunction(
              builder, "__san_cov_flush", voidType, {});
    ...
    dumpFunc(func);
    LogInfo::MapleLogger() << "ASAN done instrumenting: " << functionModified << " "
                           << func.GetName() << "\n";
  ...
  std::vector<MemoryAccess> AddressSanitizer::isInterestingMemoryAccess(StmtNode *stmtNode) {
    LogInfo::MapleLogger() << "ASAN isInterestingMemoryAccess " \
        << stmtNode->GetSrcPos().DumpLocWithColToString() << "\n";
    ...
    while (!baseNodeStack.empty()) {
      ...
      switch(baseNode->GetOpCode()) {
        case OP_iassign: {
          ...
          BaseNode *addr = module->GetMIRBuilder()->CreateExprIaddrof(
                  PTY_u64, iassign->GetTyIdx(), iassign->GetFieldID(),iassign->Opnd(0));
          struct MemoryAccess memoryAccess = {stmtNode, true, pointedTy->GetSize() << 3,
                  align, addr};
        ...
        case OP_iassignoff: {
          ...
          if (offset == 0) {
            ...
            MIRType *mirType = module->CurFuncCodeMemPool()->New<MIRType>(
              MIRTypeKind::kTypePointer, addrPrimType);
            MIRIntConst *offsetConstVal = module->CurFuncCodeMemPool()->New<MIRIntConst>(
              offset, *mirType);
            ConstvalNode *offsetNode = module->CurFuncCodeMemPool()->New<ConstvalNode>(
              addrPrimType);
            offsetNode->SetConstVal(offsetConstVal);
            addrExpr = module->CurFuncCodeMemPool()->New<BinaryNode>(
              OP_add, addrPrimType, addrNode, offsetNode);
          }
          BaseNode *addr = module->GetMIRBuilder()->CreateExprIaddrof(
            addrPrimType, TyIdx(primType), 0, addrExpr);
          struct MemoryAccess memoryAccess = {stmtNode, true, primTypeSize << 3,
                  primTypeSize, addr};
          memAccess.emplace_back(memoryAccess);
          break;
        }
        ...
        case OP_iread: {
          ...
          BaseNode *addr = module->GetMIRBuilder()->CreateExprIaddrof(
                  PTY_u64, iread->GetTyIdx(), iread->GetFieldID(), iread->Opnd(0));
          struct MemoryAccess memoryAccess = {stmtNode, false,
                  pointedTy->GetSize() << 3, align, addr};
          memAccess.emplace_back(memoryAccess);
          break;
        }
        case OP_ireadoff:
  ...
  void AddressSanitizer::initializeCallbacks(MIRModule &module) {
    ...
    MIRType *Int8PtrType = GlobalTables::GetTypeTable().GetOrCreatePointerType(
            GlobalTables::GetTypeTable().GetInt8()->GetTypeIndex());

#ifdef ENABLERBTREE
    AsanRBTSafetyCheck = getOrInsertFunction(
            mirBuilder, "__asan_rbt_safety_check", voidType, {IntPtrTy, Int32Type});
    AsanRBTStackInsert = getOrInsertFunction(
            mirBuilder, "__asan_rbt_stack_insert", voidType, {IntPtrTy, Int32Type});
    AsanRBTStackDelete = getOrInsertFunction(
            mirBuilder, "__asan_rbt_stack_delete", voidType, {IntPtrTy, Int32Type});
#else
    for (size_t AccessIsWrite = 0; AccessIsWrite <= 1; AccessIsWrite++) {
      ...
      for (size_t AccessSizeIndex = 0; AccessSizeIndex < kNumberOfAccessSizes; AccessSizeIndex++) {
        ...
        AsanErrorCallback[AccessIsWrite][AccessSizeIndex] = getOrInsertFunction(
                mirBuilder, (kAsanReportErrorTemplate + Suffix).c_str(), voidType, {IntPtrTy});
      }
    }
#endif

    AsanMemmove = getOrInsertFunction(
            mirBuilder, "__asan_memmove", Int8PtrType, {Int8PtrType, Int8PtrType, IntPtrTy});
    AsanMemcpy = getOrInsertFunction(
            mirBuilder,  "__asan_memcpy", Int8PtrType, {Int8PtrType, Int8PtrType, IntPtrTy});
    AsanMemset = getOrInsertFunction(
            mirBuilder, "__asan_memset", Int8PtrType, {Int8PtrType, Int32Type, IntPtrTy});
  ...
  BaseNode* AddressSanitizer::memToShadow(BaseNode* Shadow, MIRBuilder &mirBuilder) {
    Shadow = mirBuilder.CreateExprBinary(OP_ashr, *GlobalTables::GetTypeTable().GetInt64(),
                                         Shadow, mirBuilder.CreateIntConst(Mapping.Scale, IntPtrPrim));
    if (Mapping.Offset == 0) {
      return Shadow;
    }
    BaseNode *ShadowBase;
  ...
  void AddressSanitizer::instrumentAddress(StmtNode *OrigIns, StmtNode *InsertBefore, BaseNode *Addr,
    ...
    MIRBuilder *mirBuilder = module->GetMIRBuilder();
      ...
      func->GetMirFunc()->GetBody()->InsertBefore(InsertBefore, mirBuilder->CreateStmtCall(AsanRBTSafetyCheck->GetPuidx(), args));
      return;
#endif

    size_t accessSizeIndex = TypeSizeToSizeIndex(TypeSize);
    MIRSymbol *addrSymbol = getOrCreateSymbol(mirBuilder, IntPtrTy->GetTypeIndex(), "asan_addr",
                                                     kStVar, kScAuto, module->CurFunction(), kScopeLocal);
    ...
    MIRPtrType *shadowPtrTy = dynamic_cast<MIRPtrType *>(
            GlobalTables::GetTypeTable().GetOrCreatePointerType(shadowTy->GetTypeIndex()));
    ...
    MIRSymbol *shadowValue = getOrCreateSymbol(mirBuilder, shadowTy->GetTypeIndex(), "asan_shadowValue",
                                                           kStVar, kScAuto, module->CurFunction(), kScopeLocal);
    ...
    // Check if value != 0
    BinaryNode *cmp = mirBuilder->CreateExprBinary(OP_ne, *shadowTy,
                                                   mirBuilder->CreateDread(*shadowValue, shadowTy->GetPrimType()), cmpVal);
  ...
  void AddressSanitizer::instrumentUnusualSizeOrAlignment(StmtNode *I, StmtNode *InsertBefore,
                                                          BaseNode *Addr, uint32_t TypeSize, bool IsWrite) {
    MIRBuilder *mirBuilder = module->GetMIRBuilder();
    ...
    DassignNode *dassignNode = mirBuilder->CreateStmtDassign(addrSymbol->GetStIdx(), 0, Addr);
    dassignNode->InsertAfterThis(*InsertBefore);
    BinaryNode *binaryNode = mirBuilder->CreateExprBinary(
            OP_add, *IntPtrTy, mirBuilder->CreateDread(*addrSymbol, IntPtrPrim),
            mirBuilder->CreateIntConst(TypeSize / 8 - 1, IntPtrPrim));
    MIRSymbol *lastByteSymbol = getOrCreateSymbol(mirBuilder, IntPtrTy->GetTypeIndex(), "asan_lastByte",
                                                     kStVar, kScAuto, module->CurFunction(), kScopeLocal);
  ...
  BinaryNode *AddressSanitizer::createSlowPathCmp(StmtNode *InsBefore, BaseNode *AddrLong,
                                                  BaseNode *ShadowValue, uint32_t TypeSize) {
    MIRBuilder *mirBuilder = module->GetMIRBuilder();
    size_t granularity = static_cast<size_t>(1) << Mapping.Scale;
    // Addr & (Granularity - 1)
    BinaryNode *lastAccessedByte = mirBuilder->
            CreateExprBinary(OP_band, *IntPtrTy,
                             AddrLong,
                             mirBuilder->CreateIntConst(granularity - 1, IntPtrPrim));
    // (Addr & (Granularity - 1)) + size - 1
    if (TypeSize / 8 > 1) {
      lastAccessedByte = mirBuilder->
              CreateExprBinary(OP_add, *IntPtrTy, lastAccessedByte,
                               mirBuilder->CreateIntConst(TypeSize / 8 - 1, IntPtrPrim));
    }
    // (uint8_t) ((Addr & (Granularity-1)) + size - 1)
    MIRSymbol *length = getOrCreateSymbol(mirBuilder, (TyIdx) ShadowValue->GetPrimType(), "asan_length",
                                                      kStVar, kScAuto, module->CurFunction(), kScopeLocal);
    ...
    return mirBuilder->CreateExprBinary(
            OP_ge, *GlobalTables::GetTypeTable().GetTypeFromTyIdx(ShadowValue->GetPrimType()),
            mirBuilder->CreateDread(*length, ShadowValue->GetPrimType()), ShadowValue);
  }

  CallNode* AddressSanitizer::generateCrashCode(MIRSymbol* Addr, bool IsWrite, size_t AccessSizeIndex, BaseNode *SizeArgument) {
  ...
  StmtNode* AddressSanitizer::splitIfAndElseBlock(Opcode op, StmtNode *elsePart, const BinaryNode *cmpStmt) {
    MIRBuilder *mirBuilder = module->GetMIRBuilder();
    auto *cmpNode = mirBuilder->CreateExprCompare(cmpStmt->GetOpCode(),
                                                 *GlobalTables::GetTypeTable().GetTypeFromTyIdx(TyIdx(cmpStmt->GetPrimType())),
                                                 *GlobalTables::GetTypeTable().GetTypeFromTyIdx(TyIdx(cmpStmt->GetPrimType())),
                                                  cmpStmt->Opnd(0), cmpStmt->Opnd(1));
  ...
  bool AddressSanitizer::isInterestingSymbol(MIRSymbol &symbol) {
    ...
    if (std::find(preAnalysis->usedInAddrof.begin(),
                  preAnalysis->usedInAddrof.end(), &symbol) != preAnalysis->usedInAddrof.end()) {
      return true;
    }
