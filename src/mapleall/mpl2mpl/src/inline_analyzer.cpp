...
static constexpr int64 kBigFuncNumInsns = 100000;

static int64 HandleSwitch(StmtNode &stmt, InlineAnalyzer&) {
  SwitchNode &switchNode = static_cast<SwitchNode&>(stmt);
  return static_cast<int64>((switchNode.GetSwitchTable().size() + 1) * kSizeScale);
}

static int64 HandleCall(StmtNode &stmt, InlineAnalyzer &ia) {
  int64 cost = 0;
  CallNode &callNode = static_cast<CallNode&>(stmt);
  PUIdx puIdx = callNode.GetPUIdx();
...
static int64 HandleIcall(StmtNode &stmt, InlineAnalyzer &ia) {
  int64 cost = 0;
...
static int64 HandleDassign(StmtNode &stmt, InlineAnalyzer &ia) {
  int64 cost = 0;
...
static int64 HandleIassign(StmtNode &stmt, InlineAnalyzer &ia) {
  int64 cost = 0;
...
static int64 HandleIf(StmtNode &stmt, InlineAnalyzer &ia) {
  int64 cost = 0;
...
static int64 HandleWhile(StmtNode &stmt, InlineAnalyzer &ia) {
  int64 cost = 0;
...
static int64 HandleIntrinsicCall(StmtNode &stmt, InlineAnalyzer &ia) {
...
using GetStmtCostFactory = FunctionFactory<Opcode, int64, StmtNode&, InlineAnalyzer&>;
...
void InlineAnalyzer::Init() {
  ti = TargetInfo::CreateTargetInfo(alloc);
  InitStmtCostFactory();
}

int64 InlineAnalyzer::GetStmtsCost(BlockNode *block) {
  if (block == nullptr) {
    return 0;
  }
  int64 cost = 0;
...
int64 InlineAnalyzer::GetMoveCost(size_t sizeInByte) {
...
int64 InlineAnalyzer::GetMeExprCost(MeExpr *meExpr) {
  if (meExpr == nullptr) {
    return 0;
  }
...
int64 InlineAnalyzer::GetMeStmtCost(MeStmt *meStmt) {
  if (meStmt == nullptr) {
    return 0;
  }
...
int64 InlineAnalyzer::GetExprCost(BaseNode *expr) {
  if (expr == nullptr) {
    return 0;
  }
  int64 cost = 0;
...
MIRType *InlineAnalyzer::GetMIRTypeFromStIdxAndField(const StIdx idx, FieldID fieldID) {
...
int64 InlineAnalyzer::GetStmtCost(StmtNode *stmt) {
  if (stmt == nullptr) {
    return 0;
  }
