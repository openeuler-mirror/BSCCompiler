...
void CallGraph::ClearFunctionList() const {
...
void CallGraph::UpdateCaleeCandidate(PUIdx callerPuIdx, const IcallNode *icall,
                                     PUIdx calleePuidx, CallNode *call) const {
  CGNode *caller = GetCGNode(callerPuIdx);
  for (auto &pair : std::as_const(caller->GetCallee())) {
    auto *callsite = pair.first;
    if (callsite->GetCallStmt() == icall) {
...
void IPODevirtulize::SearchDefInClinit(const Klass &klass) const {
...
static constexpr uint32 kMaxPropTempDepth = 10;  // Fine tuning is possible here

// Propagate the temperature with `cgNode` as the starting point
void PropTempForCGNode(CGNode &cgNode, uint32 curPropTempDepth, double exeCnt, uint32 indent, bool debug) {
  if (curPropTempDepth > kMaxPropTempDepth) {
    return;
  }
  ...
  for (auto pair : calleeMap) {
    ...
    if (caller == calleeFunc) {
      continue;  // Skip self-recursive callsite to avoid infinite loop
    }
    ...
    PropTempForCGNode(*calleeNode, curPropTempDepth + 1, calleeExeCnt, indent + numSpaces, debug);
  }
}
...
void CallGraph::PropTempAcrossCallGraph() {
  ...
  for (size_t index = 0; index < funcTable.size(); ++index) {
    ...
    PropTempForCGNode(*cgNode, 0, exeCntForHotFunc, 0, debugFlag);
  }
}
...
void CallGraph::SetCompilationFunclist() const {
  std::set<MIRFunction*> unusedFuncSet(mirModule->GetFunctionList().cbegin(),
      mirModule->GetFunctionList().cend());
