...
void MInline::InitParams() {
  ...
  } else if (Options::enableGInline) {
    // use new cost model, scale down the thresholds
    smallFuncThreshold /= kScaleDownForNewCostModel;
...
void MInline::InlineCallsBlockInternal(MIRFunction &func, BaseNode &baseNode, bool &changed) {
  ...
  std::pair<bool, InlineFailedCode> canInlineRet = InlineAnalyzer::CanInlineImpl(func, *callee, callStmt, *cg);
