...
bool MayBeOverwritten(const MIRFunction &func) {
  if (!Options::symbolInterposition) {
    return false;
  }
  if (func.IsStatic()) {
    return false;
  }
  if (func.GetAttr(FUNCATTR_always_inline)) {
    return false;
  }
  if (IsInlineDefinition(func) || IsExternGnuInline(func)) {
    return false;
  }
  // for now, we only take pic&pie into account.
