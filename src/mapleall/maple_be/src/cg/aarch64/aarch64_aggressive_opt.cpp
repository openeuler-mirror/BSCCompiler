...
bool AArch64CombineRedundantX16Opt::IsEndOfSegment(const Insn &insn, bool hasX16Def) {
  ...
  // For corner case:
  //   mov R16, #4097        ---->   $recentX16DefInsn that does not have recentSplitUseOpnd
  //   use R16 (e.g.  sub R10, R10, R16)
  //   add R16, sp, #12288   ---->   $curInsn that define x16
  //
  // But for the following cases, they are not the end of segment:
  // (1)
  //   mov R16, #4097        ---->   $recentX16DefPrevInsns
  //   add R16, R10, R16     ---->   $recentX16DefInsn
  //   use R16
  //   add R16, R10, #12288  ---->   $curInsn that define x16
  // (2)
  //   mov R16, #4097        ---->   $recentX16DefInsn that does not have recentSplitUseOpnd
  //   add R16, R16, #12288  ---->   $curInsn that define x16
  if (hasX16Def && recentX16DefInsn != nullptr &&
      (recentX16DefInsn->GetMachineOpcode() == MOP_wmovri32 || recentX16DefInsn->GetMachineOpcode() == MOP_xmovri64) &&
      useOpnd != nullptr && useOpnd->GetRegisterNumber() != R16) {
    return true;
  }
  return false;
}
  ...
  for (uint32 i = 0; i < opndNum; ++i) {
    ...
    switch (opnd.GetKind()) {
      ...
      case Operand::kOpdList: {
        for (const auto *opndElem : static_cast<ListOperand&>(opnd).GetOperands()) {
