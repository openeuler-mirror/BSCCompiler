...
MIRStructType *AArch64CGFunc::GetLmbcStructArgType(BaseNode &stmt, int32 argNo) {
  MIRType *ty = nullptr;
  if (stmt.GetOpCode() == OP_call) {
    ...
    if (callFunc->GetFormalCount() < (argNo + 1)) {
    ...
    if (fType->GetParamTypeList().size() < (argNo + 1)) {
      return nullptr;
    }
...
MemOperand &AArch64CGFunc::ConstraintOffsetToSafeRegion(uint32 bitLen, const MemOperand &memOpnd) {
  ...
  int32 multiplier = (offsetValue / k512BitSizeInt) + static_cast<int32>(offsetValue % k512BitSizeInt > k256BitSizeInt);
...
void AArch64CGFunc::SelectIassignspoff(PrimType pTy, int32 offset, Operand &opnd) {
  ...
  if (curRegArgs < static_cast<int32>(k8ByteSize)) {
    ...
    GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(PickStInsn(bitLen, pTy),
                                                                  reg, memOpd));
  }
...
bool AArch64CGFunc::LmbcSmallAggForRet(BaseNode &bNode, Operand *src) {
  PrimType pTy;
  uint32 size = 0;
  ...
  if (func->IsReturnStruct()) {
    ...
    uint32 loadSize;
    uint32 numRegs = 0;
    if (static_cast<StmtNode&>(bNode).GetNext()->GetOpCode() == OP_return) {
...
Operand *AArch64CGFunc::SelectIreadoff(const BaseNode &parent, IreadoffNode &ireadoff) {
  ...
  if (primType == PTY_agg && parent.GetOpCode() == OP_regassign) {
    ...
    LmbcSmallAggForRet(const_cast<BaseNode&>(parent), addrOpnd);
    // result not used
  } else {
    auto &memOpnd = CreateMemOpnd(LoadIntoRegister(*addrOpnd, PTY_a64), offset, bitSize);
...
Operand *AArch64CGFunc::SelectIreadfpoff(const BaseNode &parent, IreadFPoffNode &ireadoff) {
  ...
  RegOperand *result = nullptr;
  if (offset > 0) {
    CHECK_FATAL(0, "Invalid ireadfpoff offset");
  } else {
    if (primType == PTY_agg) {
      /* agg return */
  ...
  if (opnd1Type == Operand::kOpdRegister) {
    ...
    if (bitNum <= k16ValidBit && regNO0 != RSP) {
      ...
      BitShiftOperand &bitShiftOpnd =
          CreateBitShiftOperand(BitShiftOperand::kShiftLSL, static_cast<uint32>(tail0bitNum), bitLen);
...
void AArch64CGFunc::SelectAArch64CSet(Operand &res, CondOperand &cond, bool is64Bits) {
  ...
  Operand &rflag = GetOrCreateRflag();
  GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(mOpCode, res, cond, rflag));
}
...
void AArch64CGFunc::SelectAArch64Select(Operand &dest, Operand &opnd0, Operand &opnd1, CondOperand &cond,
  ...
  if (opnd1.IsImmediate()) {
    uint32 movOp = (is64bits == k64BitSize ? MOP_xmovri64 : MOP_xmovri32);
    RegOperand &movDest = CreateVirtualRegisterOperand(
        NewVReg(kRegTyInt, (is64bits == k64BitSize) ? k8ByteSize : k4ByteSize));
    GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(movOp, movDest, opnd1));
    GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(mOpCode, dest, opnd0, movDest, cond, rflag));
    return;
  }
  GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(mOpCode, dest, opnd0, opnd1, cond, rflag));
}
...
void AArch64CGFunc::SelectParmListIreadSmallAggregate(BaseNode &iread, MIRType &structType,
  ...
  if (iread.GetOpCode() == OP_iread) {
    ...
    GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(MOP_xaddrri12, *addrOpnd0, *rfp, immOpnd));
  ...
  if (ploc.reg0 == 0) {
    ...
    if (ploc.reg1 > 0) {
      OfstOperand *offOpnd1 = &GetOrCreateOfstOpnd((((ploc.fpSize > 0) ? ploc.fpSize : kSizeOfPtr) +
    ...
    if (ploc.reg3 > 0) {
      OfstOperand *offOpnd3 = &GetOrCreateOfstOpnd((((ploc.fpSize > 0) ? (ploc.fpSize * k8BitShift) : kSizeOfPtr) +
          static_cast<uint32>(offset)), k32BitSize);
...
void AArch64CGFunc::SelectParmListForAggregate(BaseNode &parent, BaseNode &argExpr, ListOperand &srcOpnds,
...
void AArch64CGFunc::SelectParmListPreprocessLargeStruct(BaseNode &parent, BaseNode &argExpr, int32 &structCopyOffset, int32 argNo) {
  uint64 symSize;
  int32 rhsOffset = 0;
  if (argExpr.GetOpCode() == OP_dread) {
    DreadNode &dread = static_cast<DreadNode &>(argExpr);
    ...
    MIRStructType *ty = GetLmbcStructArgType(parent, argNo);
    symSize = GetBecommon().GetTypeSize(ty->GetTypeIndex());
    if (symSize > 16 /*kParmMemcpySize*/) {
      RegOperand *rfp = &GetOrCreatePhysicalRegisterOperand(RFP, k64BitSize, kRegTyInt);
      RegOperand *addrOpnd = &CreateRegisterOperandOfType(PTY_a64);
      ImmOperand &immOpnd = CreateImmOperand(ireadoff.GetOffset(), k32BitSize, true);
      GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(MOP_xaddrri12, *addrOpnd, *rfp, immOpnd));
      CreateCallStructParamMemcpy(nullptr, addrOpnd, static_cast<uint32>(symSize), structCopyOffset, 0);
      structCopyOffset += static_cast<int32>(RoundUp(symSize, kSizeOfPtr));
    }
  } else if (argExpr.GetOpCode() == OP_ireadoff) {
    ...
    if (symSize > 16 /*kParmMemcpySize*/) {
      ...
      CreateCallStructParamMemcpy(nullptr, addrOpnd,
        static_cast<uint32>(symSize), structCopyOffset, 0);
      structCopyOffset += static_cast<int32>(RoundUp(symSize, kSizeOfPtr));
    }
  }
}
...
void AArch64CGFunc::SelectParmList(StmtNode &naryNode, ListOperand &srcOpnds, bool isCallNative) {
  ...
  for (uint32 pnum = 0; i < naryNode.NumOpnds(); ++i, ++pnum) {
    ...
    /* use alloca  */
    if (primType == PTY_agg) {
      SelectParmListForAggregate(naryNode, *argExpr, srcOpnds, parmLocator, structCopyOffset, i);
      continue;
    }
...
Operand *AArch64CGFunc::GetBaseReg(const AArch64SymbolAlloc &symAlloc) {
  ...
  if (fsp == nullptr) {
    if (GetMirModule().GetFlavor() == MIRFlavor::kFlavorLmbc) {
      fsp = &GetOrCreatePhysicalRegisterOperand(RSP, kSizeOfPtr * kBitsPerByte, kRegTyInt);
    } else {
...
int32 AArch64CGFunc::GetBaseOffset(const SymbolAlloc &symbolAlloc) {
  ...
  if (sgKind == kMsArgsStkPassed) {  /* for callees */
    ...
    int32 baseOffset;
    if (GetCG()->IsLmbc()) {
      baseOffset = symAlloc->GetOffset() + memLayout->GetSizeOfRefLocals() +
                   memLayout->SizeOfArgsToStackPass();   /* SP relative */
    } else {
      baseOffset = memLayout->GetSizeOfLocals() + symAlloc->GetOffset() +
    ...
    int32 baseOffset;
    if (GetCG()->IsLmbc()) {
      baseOffset = symAlloc->GetOffset() + memLayout->SizeOfArgsRegisterPassed() +
                   memLayout->GetSizeOfRefLocals() +
...
void AArch64CGFunc::SelectStackSave() {
  ...
  Insn &saveInsn = GetCG()->BuildInstruction<AArch64Insn>(MOP_xmovrr, r0Opnd, spOpnd);
...
void AArch64CGFunc::SelectStackRestore(const IntrinsiccallNode &intrnNode) {
  ...
  Operand *opnd0 = HandleExpr(intrnNode, *argexpr0);
  Operand &spOpnd = GetOrCreatePhysicalRegisterOperand(RSP, k64BitSize, kRegTyInt);
  Insn &restoreInsn = GetCG()->BuildInstruction<AArch64Insn>(MOP_xmovrr, spOpnd, *opnd0);
