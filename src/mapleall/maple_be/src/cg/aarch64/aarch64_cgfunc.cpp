...
void AArch64CGFunc::SelectAggIassign(IassignNode &stmt, Operand &addrOpnd) {
  ...
  if (stmt.GetRHS()->GetOpCode() == OP_dread) {
    ...
    uint32 lhsSizeNotCovered = static_cast<uint32>(lhsSize - lhsSizeCovered);
...
Operand *AArch64CGFunc::SelectIread(const BaseNode &parent, IreadNode &expr,
  ...
  MemOperand *memOpnd =
      CreateMemOpndOrNull(destType, expr, *expr.Opnd(0), static_cast<int>(offset) + extraOffset, memOrd);
...
void AArch64CGFunc::SelectParmListPreprocessForAggregate(BaseNode &argExpr, int32 &structCopyOffset,
  ...
  structCopyOffset = static_cast<int32>(RoundUp(static_cast<uint32>(structCopyOffset), align));
