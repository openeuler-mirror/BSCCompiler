...
void AArch64CGFunc::SelectCopyRegOpnd(Operand &dest, PrimType dtype, Operand::OperandType opndType,
  ...
  if (opndType != Operand::kOpdMem) {
    ...
    GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(PickMovBetweenRegs(dtype, stype), dest, src));
    return;
  }
  AArch64isa::MemoryOrdering memOrd = AArch64isa::kMoNone;
  ...
  switch (opnd1Type) {
    ...
    case Operand::kOpdRegister: {
      if (opnd0Type == Operand::kOpdRegister && IsPrimitiveVector(stype)) {
        ...
        Insn *insn = &GetCG()->BuildInstruction<AArch64VectorInsn>(mop, dest, src);
        VectorRegSpec *vecSpecSrc = GetMemoryPool()->New<VectorRegSpec>(dsize >> k3ByteSize, k8BitSize);
        VectorRegSpec *vecSpecDest = GetMemoryPool()->New<VectorRegSpec>(dsize >> k3ByteSize, k8BitSize);
 ...
 * adjust the offsets.
 */
bool AArch64CGFunc::IsImmediateOffsetOutOfRange(const AArch64MemOperand &memOpnd, uint32 bitLen) {
 ...
 */
AArch64MemOperand &AArch64CGFunc::ConstraintOffsetToSafeRegion(uint32 bitLen, AArch64MemOperand &memOpnd) {
  ...
  AArch64MemOperand &newMemOpnd = CreateReplacementMemOperand(bitLen, *resOpnd, newOffset);
...
AArch64MemOperand &AArch64CGFunc::SplitOffsetWithAddInstruction(const AArch64MemOperand &memOpnd, uint32 bitLen,
  ...
  if (addend > 0) {
    ...
    if (!AArch64MemOperand::IsPIMMOffsetOutOfRange(static_cast<int32>(remainedTmp), bitLen) &&
  ...
  AArch64MemOperand &newMemOpnd = CreateReplacementMemOperand(bitLen, *resOpnd, remained);
  newMemOpnd.SetStackMem(memOpnd.IsStackMem());
  return newMemOpnd;
}
...
RegOperand *AArch64CGFunc::ExtractNewMemBase(const MemOperand &memOpnd) {
  ...
  AArch64MemOperand::AArch64AddressingMode mode = static_cast<const AArch64MemOperand*>(&memOpnd)->GetAddrMode();
  ...
  if (mode == AArch64MemOperand::kAddrModeLo12Li) {
    ...
    AArch64OfstOperand *offsetOpnd = static_cast<const AArch64MemOperand*>(&memOpnd)->GetOffsetImmediate();
    ...
    RegOperand *regOpnd = static_cast<const AArch64MemOperand*>(&memOpnd)->GetOffsetRegister();
...
void AArch64CGFunc::SelectDassign(StIdx stIdx, FieldID fieldId, PrimType rhsPType, Operand &opnd0) {
  ...
  memOpnd = a64MemOpnd->IsOffsetMisaligned(dataSize) ? &ConstraintOffsetToSafeRegion(dataSize, *a64MemOpnd) : memOpnd;
...
void AArch64CGFunc::SelectDassignoff(DassignoffNode &stmt, Operand &opnd0) {
  ...
  auto *a64MemOpnd = static_cast<AArch64MemOperand*>(memOpnd);
  memOpnd = a64MemOpnd->IsOffsetMisaligned(size) ? &ConstraintOffsetToSafeRegion(size, *a64MemOpnd) : memOpnd;
  Insn &insn = GetCG()->BuildInstruction<AArch64Insn>(mOp, stOpnd, *memOpnd);
  GetCurBB()->AppendInsn(insn);
}
...
AArch64MemOperand *AArch64CGFunc::FixLargeMemOpnd(MOperator mOp, MemOperand &memOpnd, uint32 dSize, uint32 opndIdx) {
  auto *a64MemOpnd = static_cast<AArch64MemOperand*>(&memOpnd);
...
AArch64MemOperand *AArch64CGFunc::GenLargeAggFormalMemOpnd(const MIRSymbol &sym, uint32 align, int64 offset,
                                                           bool needLow12) {
  MemOperand *memOpnd;
...
RegOperand *AArch64CGFunc::PrepareMemcpyParamOpnd(int64 offset, Operand &exprOpnd) {
  RegOperand *tgtAddr = &CreateVirtualRegisterOperand(NewVReg(kRegTyInt, k8ByteSize));
  AArch64OfstOperand *ofstOpnd = &GetOrCreateOfstOpnd(static_cast<uint64>(offset), k32BitSize);
  GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(MOP_xaddrri12, *tgtAddr, exprOpnd, *ofstOpnd));
  return tgtAddr;
}
...
RegOperand *AArch64CGFunc::PrepareMemcpyParamOpnd(uint64 copySize) {
  ...
  AArch64ImmOperand *sizeOpnd = &CreateImmOperand(static_cast<int64>(copySize), k64BitSize, false);
...
void AArch64CGFunc::SelectAggDassign(DassignNode &stmt) {
  ...
  if (stmt.GetRHS()->GetOpCode() == OP_dread) {
    ...
    for (uint32 i = 0; i < (lhsSize / copySize); i++) {
      ...
      AArch64MemOperand::AArch64AddressingMode addrMode =
          rhsIsLo12 ? AArch64MemOperand::kAddrModeLo12Li : AArch64MemOperand::kAddrModeBOi;
    ...
    while (lhsSizeCovered < lhsSize) {
      ...
      AArch64MemOperand::AArch64AddressingMode addrMode =
          rhsIsLo12 ? AArch64MemOperand::kAddrModeLo12Li : AArch64MemOperand::kAddrModeBOi;
      MIRSymbol *sym = rhsIsLo12 ? rhsSymbol : nullptr;
      AArch64OfstOperand &rhsOfstOpnd = GetOrCreateOfstOpnd(lhsSizeCovered + static_cast<uint64>(rhsOffsetVal),
                                                            k32BitSize);
      ...
      addrMode = lhsIsLo12 ? AArch64MemOperand::kAddrModeLo12Li : AArch64MemOperand::kAddrModeBOi;
      sym = lhsIsLo12 ? lhsSymbol : nullptr;
      AArch64OfstOperand &lhsOfstOpnd = GetOrCreateOfstOpnd(lhsSizeCovered + static_cast<uint64>(lhsOffsetVal),
                                                            k32BitSize);
      MemOperand *lhsMemOpnd;
    ...
    for (uint32 i = 0; i < (lhsSize / copySize); i++) {
      ...
      if (doPair) {
        ...
        rhsMemOpnd = FixLargeMemOpnd(mOpLDP, *rhsMemOpnd, copySize * k8BitSize, kInsnThirdOpnd);
        insn = &GetCG()->BuildInstruction<AArch64Insn>(mOpLDP, result, *result1, *rhsMemOpnd);
      } else {
        MOperator mOp = PickLdInsn(copySize * k8BitSize, PTY_u32);
        rhsMemOpnd = FixLargeMemOpnd(mOp, *rhsMemOpnd, copySize * k8BitSize, kInsnSecondOpnd);
      ...
      /* generate the store */
      AArch64MemOperand::AArch64AddressingMode addrMode =
          lhsIsLo12 ? AArch64MemOperand::kAddrModeLo12Li : AArch64MemOperand::kAddrModeBOi;
...
void AArch64CGFunc::SelectAggIassign(IassignNode &stmt, Operand &AddrOpnd) {
  ...
  if (stmt.GetRHS()->GetOpCode() == OP_dread) {
    ...
    while (lhsSizeCovered < lhsSize) {
      ...
      MemOperand &lhsMemOpnd = GetOrCreateMemOpnd(AArch64MemOperand::kAddrModeBOi, newAlignUsed * k8BitSize,
    ...
    while (lhsSizeCovered < lhsSize) {
      ...
      MemOperand *rhsMemOpnd = &GetOrCreateMemOpnd(AArch64MemOperand::kAddrModeBOi, memOpndSize,
          static_cast<AArch64RegOperand*>(rhsAddrOpnd), nullptr, &rhsOfstOpnd, nullptr);
      regno_t vRegNO = NewVReg(kRegTyInt, std::max(4u, newAlignUsed));
      RegOperand &result = CreateVirtualRegisterOperand(vRegNO);
      ...
      Insn &insn = GetCG()->BuildInstruction<AArch64Insn>(mOpLD, result, *rhsMemOpnd);
      insn.MarkAsAccessRefField(isRefField);
      GetCurBB()->AppendInsn(insn);
      /* generate the store */
      AArch64OfstOperand &lhsOfstOpnd = GetOrCreateOfstOpnd(lhsOffset + lhsSizeCovered, k32BitSize);
      MemOperand *lhsMemOpnd = &GetOrCreateMemOpnd(AArch64MemOperand::kAddrModeBOi, memOpndSize,
          static_cast<AArch64RegOperand*>(&lhsAddrOpnd), nullptr, &lhsOfstOpnd, nullptr);
      ...
      GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(mOpST, result, *lhsMemOpnd));
      lhsSizeCovered += newAlignUsed;
    }
  }
}
  ...
  if ((symbol->GetStorageClass() == kScAuto) || (symbol->GetStorageClass() == kScFormal)) {
    ...
    if (CGOptions::IsPIC() && symbol->NeedPIC()) {
      ...
      AArch64MemOperand &memOpnd = GetOrCreateMemOpnd(AArch64MemOperand::kAddrModeBOi, kSizeOfPtr * kBitsPerByte,
          static_cast<AArch64RegOperand*>(srcOpnd), nullptr, &offset, nullptr);
      GetCurBB()->AppendInsn(
      ...
      GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(MOP_xadrpl12, result, *srcOpnd, stImm));
    }
  }
}
...
Operand *AArch64CGFunc::SelectAddrof(AddrofNode &expr, const BaseNode &parent) {
  MIRSymbol *symbol = GetFunction().GetLocalOrGlobalSymbol(expr.GetStIdx());
  int32 offset = 0;
  if (expr.GetFieldID() != 0) {
    MIRStructType *structType = static_cast<MIRStructType*>(symbol->GetType());
...
Operand *AArch64CGFunc::SelectIreadoff(const BaseNode &parent, IreadoffNode &ireadoff) {
  ...
  GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(mop, *result, memOpnd));
  return result;
}
...
Operand *AArch64CGFunc::SelectIread(const BaseNode &parent, IreadNode &expr,
  ...
  auto *a64MemOpnd = static_cast<AArch64MemOperand*>(memOpnd);
  memOpnd = a64MemOpnd->IsOffsetMisaligned(bitSize) ? &ConstraintOffsetToSafeRegion(bitSize, *a64MemOpnd) : memOpnd;
  if (memOrd == AArch64isa::kMoNone) {
    MOperator mOp = 0;
    ...
    auto *a64MemOpnd = static_cast<AArch64MemOperand*>(memOpnd);
    if ((memOpnd->GetMemVaryType() == kNotVary) && IsImmediateOffsetOutOfRange(*a64MemOpnd, bitSize)) {
      memOpnd = &SplitOffsetWithAddInstruction(*a64MemOpnd, bitSize);
    }
...
Operand *SelectLiteral(T *c, MIRFunction *func, uint32 labelIdx, AArch64CGFunc *cgFunc) {
  ...
  if (T::GetPrimType() == PTY_f32) {
    return (fabs(c->GetValue()) < std::numeric_limits<float>::denorm_min()) ?
        static_cast<Operand*>(&cgFunc->GetOrCreateFpZeroOperand(static_cast<uint8>(typeBitSize))) :
...
void AArch64CGFunc::SelectMadd(Operand &resOpnd, Operand &opndM0, Operand &opndM1, Operand &opnd1, PrimType primType) {
  ...
  GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(mOp, resOpnd, opndM0, opndM1, opnd1));
}
...
void AArch64CGFunc::SelectAArch64CCmp(Operand &o, Operand &i, Operand &nzcv, CondOperand &cond, bool is64Bits) {
  ...
  opndVec.push_back(&rflag);
  GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(mOpCode, opndVec));
}
...
void AArch64CGFunc::SelectAArch64CSet(Operand &r, CondOperand &cond, bool is64Bits) {
  ...
  Operand &rflag = GetOrCreateRflag();
  GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(mOpCode, r, cond, rflag));
}
...
Operand *AArch64CGFunc::SelectRor(BinaryNode &node, Operand &opnd0, Operand &opnd1, const BaseNode &parent) {
  ...
  GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(mopRor, *resOpnd, *firstOpnd, opnd1));
  return resOpnd;
}
...
Operand *AArch64CGFunc::SelectAbsSub(Insn &lastInsn, const UnaryNode &node, Operand &newOpnd0) {
  ...
  GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(mopCsneg, resOpnd, newOpnd0, condOpnd, rflag));
  return &resOpnd;
}
...
Operand *AArch64CGFunc::SelectAbs(UnaryNode &node, Operand &opnd0) {
  ...
  if (IsPrimitiveVector(dtyp)) {
    ...
    Operand &rflag = GetOrCreateRflag();
    GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(mopCsneg, resOpnd, newOpnd0, newOpnd0,
                                                                  condOpnd, rflag));
    return &resOpnd;
  }
}
...
void AArch64CGFunc::SelectSelect(Operand &resOpnd, Operand &condOpnd, Operand &trueOpnd, Operand &falseOpnd,
                                 PrimType dtype, PrimType ctype, bool hasCompare, AArch64CC_t cc) {
...
void AArch64CGFunc::SelectAArch64Select(Operand &dest, Operand &o0, Operand &o1, CondOperand &cond, bool isIntType,
  ...
  if (o1.IsImmediate()) {
    ...
    GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(mOpCode, dest, o0, movDest, cond, rflag));
    return;
  }
  GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(mOpCode, dest, o0, o1, cond, rflag));
}
...
void AArch64CGFunc::SelectParmListIreadSmallAggregate(const IreadNode &iread, MIRType &structType,
  ...
  if (ploc.reg0 == 0) {
    ...
    AArch64OfstOperand *offOpnd0 = &GetOrCreateOfstOpnd(static_cast<uint64>(offset), k32BitSize);
    MemOperand *mopnd =
    ...
    if (ploc.reg1) {
      AArch64OfstOperand *offOpnd1 =
          &GetOrCreateOfstOpnd(((ploc.fpSize ? ploc.fpSize : kSizeOfPtr) + static_cast<uint32>(offset)), k32BitSize);
    ...
    if (ploc.reg2) {
      AArch64OfstOperand *offOpnd2 =
          &GetOrCreateOfstOpnd(((ploc.fpSize ? (ploc.fpSize * k4BitShift) : kSizeOfPtr) + static_cast<uint32>(offset)),
                               k32BitSize);
    ...
    if (ploc.reg3) {
      AArch64OfstOperand *offOpnd3 =
          &GetOrCreateOfstOpnd(((ploc.fpSize ? (ploc.fpSize * k8BitShift) : kSizeOfPtr) + static_cast<uint32>(offset)),
                               k32BitSize);
...
void AArch64CGFunc::SelectParmListDreadLargeAggregate(const MIRSymbol &sym, MIRType &structType,
  ...
  structCopyOffset += static_cast<int32>(numMemOp * kSizeOfPtr);
}

void AArch64CGFunc::SelectParmListIreadLargeAggregate(const IreadNode &iread, MIRType &structType,
                                                      AArch64ListOperand &srcOpnds, AArch64CallConvImpl &parmLocator,
...
AArch64RegOperand *AArch64CGFunc::SelectParmListDreadAccessField(const MIRSymbol &sym, FieldID fieldID,
  ...
  if (sym.GetStorageClass() == kScFormal && fieldID > 0) {
    ...
    memOpnd = &CreateMemOpnd(base, (static_cast<int64>(offset) + parmNum * kSizeOfPtr), memSize);
  } else if (ploc.fpSize) {
    ...
    if (CGOptions::IsArm64ilp32()) {
      ...
      memOpnd = &GetOrCreateMemOpnd(sym, (kSizeOfPtr * parmNum + static_cast<int64>(offset)), memSize);
    }
  }
...
void AArch64CGFunc::CreateCallStructParamPassByReg(regno_t regno, MemOperand &memOpnd, AArch64ListOperand &srcOpnds,
                                                   fpParamState state) {
  AArch64RegOperand *parmOpnd;
...
AArch64RegOperand *AArch64CGFunc::CreateCallStructParamCopyToStack(uint32 numMemOp, const MIRSymbol *sym,
  ...
  for (uint32 j = 0; j < numMemOp; j++) {
    if (sym != nullptr) {
      if (sym->GetStorageClass() == kScFormal) {
        ...
        ldMopnd = &GetOrCreateMemOpnd(AArch64MemOperand::kAddrModeBOi, k64BitSize, &vreg, nullptr,
            &GetOrCreateOfstOpnd((j * kSizeOfPtr + static_cast<uint64>(fromOffset)), k32BitSize), nullptr);
      } else {
        if (CGOptions::IsArm64ilp32()) {
          ldMopnd = &GetOrCreateMemOpnd(*sym, (j * kSizeOfPtr + static_cast<int64>(fromOffset)), k32BitSize);
        } else {
          ldMopnd = &GetOrCreateMemOpnd(*sym, (j * kSizeOfPtr + static_cast<int64>(fromOffset)), k64BitSize);
        }
      }
    } else {
    ...
    if (CGOptions::IsArm64ilp32()) {
      ...
      GetCurBB()->AppendInsn(cg->BuildInstruction<AArch64Insn>(PickLdInsn(k32BitSize, PTY_i32), *vreg, *ldMopnd));

      stMopnd = &CreateMemOpnd(RSP, (static_cast<int64>(copyOffset) + (j * kSizeOfPtr)), k32BitSize);
      GetCurBB()->AppendInsn(cg->BuildInstruction<AArch64Insn>(PickStInsn(k32BitSize, PTY_i32), *vreg, *stMopnd));
    } else {
      RegOperand *vreg = &CreateVirtualRegisterOperand(NewVReg(kRegTyInt, k8ByteSize));
      GetCurBB()->AppendInsn(cg->BuildInstruction<AArch64Insn>(PickLdInsn(k64BitSize, PTY_i64), *vreg, *ldMopnd));

      stMopnd = &CreateMemOpnd(RSP, (static_cast<int64>(copyOffset) + (j * kSizeOfPtr)), k64BitSize);
      GetCurBB()->AppendInsn(cg->BuildInstruction<AArch64Insn>(PickStInsn(k64BitSize, PTY_i64), *vreg, *stMopnd));
    }
  }
  ...
  if (ploc.reg0 == kRinvalid) {
    ...
    AArch64RegOperand *parmOpnd = &GetOrCreatePhysicalRegisterOperand(static_cast<AArch64reg>(ploc.reg0), k64BitSize, kRegTyInt);
...
void AArch64CGFunc::CreateCallStructMemcpyToParamReg(MIRType &structType, int32 structCopyOffset,
                                                     AArch64CallConvImpl &parmLocator, AArch64ListOperand &srcOpnds) {
  RegOperand &spReg = GetOrCreatePhysicalRegisterOperand(RSP, k64BitSize, kRegTyInt);
...
void AArch64CGFunc::SelectReturn(Operand *opnd0) {
  ...
  if (retMech.GetRegCount() > 0) {
    ...
    if (opnd0->IsRegister()) {
      ...
      AArch64RegOperand &retOpnd = GetOrCreatePhysicalRegisterOperand(retReg,
      ...
      if (!is64x1vec) {
        ...
        Insn &insn = GetCG()->BuildInstruction<AArch64Insn>(MOP_xvmovdr, retOpnd, *reg);
        GetCurBB()->AppendInsn(insn);
      }
    } else {
      CHECK_FATAL(false, "nyi");
...
AArch64RegOperand &AArch64CGFunc::GetOrCreatePhysicalRegisterOperand(std::string &asmAttr) {
...
AArch64OfstOperand &AArch64CGFunc::GetOrCreateOfstOpnd(uint64 offset, uint32 size) {
...
void AArch64CGFunc::SelectAddrofAfterRa(Operand &result, StImmOperand &stImm, std::vector<Insn *>& rematInsns) {
  ...
  if (CGOptions::IsPIC() && symbol->NeedPIC()) {
    ...
    AArch64MemOperand &memOpnd = GetOrCreateMemOpnd(AArch64MemOperand::kAddrModeBOi, kSizeOfPtr * kBitsPerByte,
                                                    ...
                                                    &offset, nullptr);
    rematInsns.emplace_back(&GetCG()->BuildInstruction<AArch64Insn>(
  ...
  if ((storageClass == kScAuto) || (storageClass == kScFormal)) {
    ...
    if (CGOptions::IsBigEndian()) {
      if (symLoc->GetMemSegment()->GetMemSegmentKind() == kMsArgsStkPassed && size < k64BitSize) {
        ...
        offsetOpnd = memPool->New<AArch64OfstOperand>(totalOffset, k64BitSize);
      }
    } else {
      offsetOpnd = memPool->New<AArch64OfstOperand>(totalOffset, k64BitSize);
    }
...
AArch64MemOperand &AArch64CGFunc::HashMemOpnd(AArch64MemOperand &tMemOpnd) {
...
MemOperand &AArch64CGFunc::CreateMemOpnd(RegOperand &baseOpnd, int64 offset, uint32 size) {
  AArch64OfstOperand &offsetOpnd = CreateOfstOpnd(static_cast<uint64>(offset), k32BitSize);
    ...
    if (addendExpr->GetOpCode() == OP_constval) {
     ...
     /* discard large offset and negative offset */
      ...
      MemOperand *memOpnd = &GetOrCreateMemOpnd(
          AArch64MemOperand::kAddrModeBOrX, GetPrimTypeBitSize(ptype),
          SelectRegread(*static_cast<RegreadNode*>(baseExpr)),
          SelectRegread(*static_cast<RegreadNode*>(addendExpr)), nullptr, nullptr);
      return memOpnd;
      /* case 4 */
      ...
      if (cvtRegreadNode->GetOpCode() == OP_regread && cvtRegreadNode->IsLeaf()) {
        ...
        bool isExpand = toSize > fromSize;
        CHECK_FATAL(toSize >= fromSize, "unKnown convert in ldr or store");
        MemOperand *memOpnd = &GetOrCreateMemOpnd(
            AArch64MemOperand::kAddrModeBOrX, GetPrimTypeBitSize(ptype),
            SelectRegread(*static_cast<RegreadNode*>(baseExpr)),
            SelectRegread(*static_cast<RegreadNode*>(cvtRegreadNode)), shiftAmount, isExpand);
        return memOpnd;
      }
    }
  }
...
MemOperand &AArch64CGFunc::CreateNonExtendMemOpnd(PrimType ptype, const BaseNode &parent, BaseNode &addrExpr,
  ...
  if ((addrExpr.GetOpCode() == OP_CG_array_elem_add) && (offset == 0) && lastInsn &&
    ...
    AArch64OfstOperand &ofstOpnd = GetOrCreateOfstOpnd(static_cast<uint64>(stOpnd.GetOffset()), k32BitSize);
    ...
    AArch64OfstOperand &ofstOpnd = GetOrCreateOfstOpnd(static_cast<uint64>(offset), k64BitSize);
...
Insn &AArch64CGFunc::GenerateGlobalLongCallAfterInsn(const MIRSymbol &func, AArch64ListOperand &srcOpnds,
  ...
  AArch64MemOperand &memOrd = GetOrCreateMemOpnd(AArch64MemOperand::kAddrModeLo12Li, kSizeOfPtr * kBitsPerByte,
                                                 ...
                                                 nullptr, &offsetOpnd, symbol);
  Insn &ldrInsn = GetCG()->BuildInstruction<AArch64Insn>(memOrd.GetSize() == k64BitSize ? MOP_xldr : MOP_wldr,
                                                         tmpReg, memOrd);
  GetCurBB()->AppendInsn(ldrInsn);
...
void AArch64CGFunc::GenCVaStartIntrin(RegOperand &opnd, uint32 stkSize) {
  ...
  if (CGOptions::IsArm64ilp32()) {
    offOpnd = &GetOrCreateOfstOpnd(kSizeOfPtr, k64BitSize);
  } else {
  ...
  if (CGOptions::IsArm64ilp32()) {
    ...
    offsOpnd2 = &CreateImmOperand(static_cast<int64>(RoundUp(static_cast<uint64>(grAreaSize), kSizeOfPtr * 2)),
                                  k64BitSize, false);
  }
  SelectSub(vReg, *offsOpnd, *offsOpnd2, LOWERED_PTR_TYPE);  /* if 1st opnd is register => sub */
  SelectAdd(vReg, stkOpnd, vReg, LOWERED_PTR_TYPE);
  offOpnd = &GetOrCreateOfstOpnd(kSizeOfPtr * 2, k64BitSize);
  strOpnd = &GetOrCreateMemOpnd(AArch64MemOperand::kAddrModeBOi, k64BitSize, &opnd, nullptr,
                                offOpnd, static_cast<MIRSymbol*>(nullptr));
  GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(
    vReg.GetSize() == k64BitSize ? MOP_xstr : MOP_wstr, vReg, *strOpnd));
...
void AArch64CGFunc::SelectCVaStart(const IntrinsiccallNode &intrnNode) {
  ...
  BaseNode *argExpr = intrnNode.Opnd(0);
  Operand *opnd = HandleExpr(intrnNode, *argExpr);
  RegOperand &opnd0 = LoadIntoRegister(*opnd, LOWERED_PTR_TYPE);  /* first argument of intrinsic */
  ...
  if (CGOptions::IsArm64ilp32()) {
    ...
    stkSize = static_cast<uint32>(RoundUp(stkSize, kSizeOfPtr));
  }
...
void AArch64CGFunc::SelectCTlsLocalDesc(Operand &result, StImmOperand &stImm) {
  ...
  GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(MOP_mrs, result, *tpidr));
  GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(MOP_tls_desc_rel, result, result, stImm));
}
...
void AArch64CGFunc::SelectCTlsGlobalDesc(Operand &result, StImmOperand &stImm) {
  ...
  AArch64OfstOperand &offset = CreateOfstOpnd(*symbol, 0, 0);
  AArch64MemOperand &memOpnd = GetOrCreateMemOpnd(AArch64MemOperand::kAddrModeBOi, kSizeOfPtr * kBitsPerByte,
      static_cast<AArch64RegOperand*>(specialFuncPtr), nullptr, &offset, nullptr);
...
Operand *AArch64CGFunc::SelectAArch64CSyncFetch(const IntrinsicopNode &intrinopNode,
  ...
  auto stldColomu = static_cast<uint32>(IsPowerOf2(static_cast<int64>(typeByteSize)));
  ...
  if (isAdd) {
    SelectAdd(*addResult, *fetchVal, *calculateEndOpnd, pty);
  } else {
    SelectSub(*addResult, *fetchVal, *calculateEndOpnd, pty);
  }
...
Operand *AArch64CGFunc::SelectCSyncCmpSwap(const IntrinsicopNode &intrinopNode, PrimType pty, bool retBool) {
  ...
  SetLab2BBMap(static_cast<int32>(clrexIdx), *atomicCleanBB);
  GetCurBB()->AppendBB(*atomicCleanBB);
  atomicCleanBB->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(MOP_clrex));
  if (retBool) {
    Operand &zero = CreateImmOperand(0, k32BitSize,false);
    Operand *operand0 = memPool->New<AArch64RegOperand>(R0, k32BitSize, kRegTyInt);
    atomicCleanBB->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(MOP_xmovri32, *operand0, zero));
  }
  ...
  SetLab2BBMap(static_cast<int32>(atomicBBLabIdx2), *atomicBB2);
...
Operand *AArch64CGFunc::SelectCSyncAddFetch(maple::IntrinsicopNode &intrinopNode, PrimType pty) {
  return SelectAArch64CSyncFetch(intrinopNode, pty, true, true);
}

Operand *AArch64CGFunc::SelectCSyncFetchAdd(IntrinsicopNode &intrinopNode, PrimType pty) {
  return SelectAArch64CSyncFetch(intrinopNode, pty, false, true);
}

Operand *AArch64CGFunc::SelectCSyncSubFetch(IntrinsicopNode &intrinopNode, PrimType pty) {
  return SelectAArch64CSyncFetch(intrinopNode, pty, true, false);
}

Operand *AArch64CGFunc::SelectCSyncFetchSub(IntrinsicopNode &intrinopNode, PrimType pty) {
  return SelectAArch64CSyncFetch(intrinopNode, pty, false, false);
}
...
Operand *AArch64CGFunc::SelectCSyncLockTestSet(IntrinsicopNode &intrinopNode, PrimType pty) {
  ...
  /* intrinsic_sync_lock_test */
  auto mop = size == k8ByteSize ? MOP_sync_lock_test_setL : MOP_sync_lock_test_setI;
  auto result = &CreateVirtualRegisterOperand(NewVReg(kRegTyInt, size));
  auto tempReg = &CreateVirtualRegisterOperand(NewVReg(kRegTyInt, size));
  auto addrReg = &LoadIntoRegister(*addrOpnd, pty);
  auto valueReg = &LoadIntoRegister(*valOpnd, pty);
  auto &label = GetOrCreateLabelOperand(CreateLabel());
  GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(mop, *result, *tempReg, *addrReg, *valueReg, label));
  return result;
}
...
RegOperand *AArch64CGFunc::SelectVectorGetHigh(PrimType rType, Operand *src) {
  ...
  RegOperand *res = &CreateRegisterOperandOfType(rType);                 /* result operand */
  VectorRegSpec *vecSpecSrc = GetMemoryPool()->New<VectorRegSpec>(k2ByteSize, k64BitSize, 1);     /* src operand */
...
RegOperand *AArch64CGFunc::SelectVectorGetLow(PrimType rType, Operand *src) {
  ...
  rType = FilterOneElementVectorType(oType);
  RegOperand *res = &CreateRegisterOperandOfType(rType);                 /* result operand */
  VectorRegSpec *vecSpecSrc = GetMemoryPool()->New<VectorRegSpec>(k2ByteSize, k64BitSize, 0);     /* src operand */
...
RegOperand *AArch64CGFunc::SelectVectorSetElement(Operand *eOpnd, PrimType eType, Operand *vOpnd,
  ...
  Insn *insn = &GetCG()->BuildInstruction<AArch64VectorInsn>(mOp, *vOpnd, *reg);
  static_cast<AArch64VectorInsn*>(insn)->PushRegSpecEntry(vecSpecSrc);
  GetCurBB()->AppendInsn(*insn);
...
void AArch64CGFunc::SelectVectorCvt(Operand *res, PrimType rType, Operand *o1, PrimType oType) {
  ...
  if (GetPrimTypeSize(rType) > GetPrimTypeSize(oType)) {
    ...
    insn = &GetCG()->BuildInstruction<AArch64VectorInsn>(MOP_vxtnuv, *res, *o1);
  } else {
...
RegOperand *AArch64CGFunc::SelectVectorCompare(Operand *o1, PrimType oty1, Operand *o2, PrimType oty2, Opcode opc) {
  ...
  if (opc == OP_lt || opc == OP_le) {
    insn = &GetCG()->BuildInstruction<AArch64VectorInsn>(mOp, *res, *o2, *o1);
  } else {
    insn = &GetCG()->BuildInstruction<AArch64VectorInsn>(mOp, *res, *o1, *o2);
  }
  static_cast<AArch64VectorInsn*>(insn)->PushRegSpecEntry(vecSpecDest);
  static_cast<AArch64VectorInsn*>(insn)->PushRegSpecEntry(vecSpec1);
  static_cast<AArch64VectorInsn*>(insn)->PushRegSpecEntry(vecSpec2);
...
RegOperand *AArch64CGFunc::SelectVectorTableLookup(PrimType rType, Operand *o1, Operand *o2) {
  ...
  Insn *insn = &GetCG()->BuildInstruction<AArch64VectorInsn>(MOP_vtbl1vvv, *res, *o1, *o2);
  static_cast<AArch64VectorInsn*>(insn)->PushRegSpecEntry(vecSpecDest);
  static_cast<AArch64VectorInsn*>(insn)->PushRegSpecEntry(vecSpec1);
  static_cast<AArch64VectorInsn*>(insn)->PushRegSpecEntry(vecSpec2);
  GetCurBB()->AppendInsn(*insn);
...
RegOperand *AArch64CGFunc::SelectVectorNarrow(PrimType rType, Operand *o1, PrimType otyp) {
  ...
  Insn *insn = &GetCG()->BuildInstruction<AArch64VectorInsn>(MOP_vxtnuv, *res, *o1);
  static_cast<AArch64VectorInsn*>(insn)->PushRegSpecEntry(vecSpecDest);
  static_cast<AArch64VectorInsn*>(insn)->PushRegSpecEntry(vecSpec1);
  GetCurBB()->AppendInsn(*insn);
  return res;
...
RegOperand *AArch64CGFunc::SelectVectorNarrow2(PrimType rType, Operand *o1, PrimType oty1, Operand *o2, PrimType oty2) {
  ...
  Insn *insn = &GetCG()->BuildInstruction<AArch64VectorInsn>(MOP_vxtn2uv, *res, *o2);
  static_cast<AArch64VectorInsn*>(insn)->PushRegSpecEntry(vecSpecDest);
  static_cast<AArch64VectorInsn*>(insn)->PushRegSpecEntry(vecSpec2);
  GetCurBB()->AppendInsn(*insn);
  return res;
...
void AArch64CGFunc::SelectVectorZip(PrimType rType, Operand *o1, Operand *o2) {
  ...
  Insn *insn1 = &GetCG()->BuildInstruction<AArch64VectorInsn>(MOP_vzip1vvv, *res1, *o1, *o2);
  static_cast<AArch64VectorInsn*>(insn1)->PushRegSpecEntry(vecSpecDest);
  static_cast<AArch64VectorInsn*>(insn1)->PushRegSpecEntry(vecSpec1);
  static_cast<AArch64VectorInsn*>(insn1)->PushRegSpecEntry(vecSpec2);
  GetCurBB()->AppendInsn(*insn1);

  Insn *insn2 = &GetCG()->BuildInstruction<AArch64VectorInsn>(MOP_vzip2vvv, *res2, *o1, *o2);
  static_cast<AArch64VectorInsn*>(insn2)->PushRegSpecEntry(vecSpecDest);
  static_cast<AArch64VectorInsn*>(insn2)->PushRegSpecEntry(vecSpec1);
  static_cast<AArch64VectorInsn*>(insn2)->PushRegSpecEntry(vecSpec2);
  GetCurBB()->AppendInsn(*insn2);

  if (GetPrimTypeSize(rType) <= k16ByteSize) {
    Operand *preg1 = &GetOrCreatePhysicalRegisterOperand(V0, k64BitSize, kRegTyFloat);
    GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(MOP_xvmovd, *preg1, *res1));
    Operand *preg2 = &GetOrCreatePhysicalRegisterOperand(V1, k64BitSize, kRegTyFloat);
    GetCurBB()->AppendInsn(GetCG()->BuildInstruction<AArch64Insn>(MOP_xvmovd, *preg2, *res2));
  }
}
