...
__Safe__ void func2(void) {
  ...
  __Safe__ {
    // CHECK: LOC [[# FILENUM]] [[# @LINE + 2 ]]
    // CHECK-NEXT: dassign %a{{.*}} (add i32 (dread i32 %a{{.*}}, constval i32 1))
    a += 1;
    // CHECK: LOC [[# FILENUM]] [[# @LINE + 2 ]]
    // CHECK-NEXT: unsafe
    __Unsafe__ {
      ...
      // CHECK: LOC [[# FILENUM]] [[# @LINE + 2 ]]
      // CHECK: dassign %a{{.*}} 0 (add i32 (dread i32 %a{{.*}}, constval i32 4))
      a += 4;
      printf("%d", a);
    // CHECK: LOC [[# FILENUM]] [[# @LINE + 2 ]]
    // CHECK: endunsafe
    ...
    // CHECK: LOC [[# FILENUM]] [[# @LINE + 2 ]]
    // CHECK: dassign %a{{.*}} 0 (add i32 (dread i32 %a{{.*}}, constval i32 5))
    a += 5;
    __Unsafe__ {
      printf("%d", a);
    }
  ...
  // CHECK: LOC [[# FILENUM]] [[# @LINE + 2 ]]
  // CHECK: dassign %a{{.*}} 0 (add i32 (dread i32 %a{{.*}}, constval i32 6))
  a += 6;
  __Unsafe__ {
    printf("%d", a);
  }
