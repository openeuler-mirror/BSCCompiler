...
int Test() {
  ...
  int res = 0;
  int *r1 = (*func1)(p, q, 20);  // CHECK-NOT: [[# @LINE ]] error:
  res += *r1;
  int *r2 = (*func2)(p, q, 40);  // CHECK-NOT: [[# @LINE ]] error:
  res += *r2;
  int *r3 = (*func3)(p, q, len * 4);  // CHECK-NOT: [[# @LINE ]] error:
  res += *r3;
  // int *r4 = (*a.func1)(p, q, 40);  // COM:CHECK: [[# @LINE ]] error:
  // res += *r4;
  // COM:CHECK: [[# @LINE + 1 ]] warning:
  // int *r5 = (*pa->func1)(p, q, 11);  // COM:CHECK: [[# @LINE ]] warning:
  // res += *r5;
  int *r6 = (*func1)(b.i, p, 20);  // CHECK-NOT: [[# @LINE ]] warning:
  res += *r6;
  // COM:CHECK: [[# @LINE + 1 ]] warning:
  // int *r7 = (*func1)(pb->i, p, 11);  // COM:CHECK: [[# @LINE ]] warning:
  // res += *r7;
  // int *r8 = (*func1)(r1, r2, 5);  // COM:CHECK: [[# @LINE ]] warning:
  int *r9 = (*func1)(r1, r3, 20);  // CHECK-NOT: [[# @LINE ]] warning:
  // COM:CHECK: [[# @LINE + 1 ]] warning:
