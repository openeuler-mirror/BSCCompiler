...
int test1() {
  ...
  // CHECK-NEXT: assertnonnull (dread ptr $g1 1)
...
int test2() {
  // CHECK: LOC [[# FILENUM]] [[# @LINE + 2 ]]{{$}}
  // CHECK-NOT: assertnonnull (dread ptr $g2 1)
  return g2.f();
}
...
int test3() {
  if (g3.f != NULL) {
    // CHECK: LOC [[# FILENUM]] [[# @LINE + 2 ]]{{$}}
    // CHECK-NEXT: assertnonnull (dread ptr $g3 1)
    return *g3.f;
  }
  return 0;
}
...
int test4() {
  // CHECK: LOC [[# FILENUM]] [[# @LINE + 2 ]]{{$}}
  // CHECK-NOT: assertnonnull (dread ptr $g4 1)
  return *g4.f;
}
...
void call_nonnull_field(g6 *f, g6 *g, g6 *h) {
  // CHECK: LOC [[# FILENUM]] [[# @LINE + 2 ]]{{$}}
  // CHECK-NEXT: callassertnonnull <&nonnull_field, 0> (dread ptr %f)
