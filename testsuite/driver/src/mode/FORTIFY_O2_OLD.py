...
compile_part = [
        C2ast(
            ...
            option="-O2 -D_FORTIFY_SOURCE=2 --target=aarch64 -U __SIZEOF_INT128__",
            infile="${APP}.c",
            outfile="${APP}.ast"
        ),
        Hir2mpl(
...
link_part = [
        Shell(
            "${OUT_ROOT}/tools/bin/aarch64-linux-gnu-gcc -L${OUT_ROOT}/tools/gcc-linaro-7.5.0/aarch64-linux-gnu/libc  -std=c89 -s -o ${APP}.out `find *.s` -lm >tmp.log 2>&1 && (cat tmp.log | tee -a compile.log && rm tmp.log) || (cat tmp.log | tee -a compile.log && rm tmp.log && exit 1)"
        )
]
