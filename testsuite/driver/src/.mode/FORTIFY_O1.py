...
compile_part = [
        MapleDriver(
            ...
            option="-O2 -D_FORTIFY_SOURCE=1 -fPIC -S --save-temps -Wno-error=int-conversion",
            redirection="compile.log"
        )
]
link_part = [
        Shell(
            "${MAPLE_BUILD_OUTPUT}/bin/maple -L${MAPLE_ROOT}/tools/gcc-linaro-7.5.0/aarch64-linux-gnu/libc  -std=c89 -s -o ${APP}.out `find *.s` -lm >tmp.log 2>&1 && (cat tmp.log | tee -a compile.log && rm tmp.log) || (cat tmp.log | tee -a compile.log && rm tmp.log && exit 1)"
        )
]
