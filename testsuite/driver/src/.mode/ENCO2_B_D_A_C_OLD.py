...
link_part = [
        Shell(
            "${MAPLE_ROOT}/tools/bin/aarch64-linux-gnu-gcc -O2 -static -L../lib/include -std=c89 -s -o a.out `find *.s` -lm -lst>tmp.log 2>&1 && (cat tmp.log | tee -a compile.log && rm tmp.log) || (cat tmp.log | tee -a compile.log && rm tmp.log && exit 1)"
        )
]
